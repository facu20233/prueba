...
private:
    ...
    bool estreno;
    float precioBase;
    std::string tipoPelicula; //Nacionales (N) o Internacionales (I).
...
public:
    ...
    pelicula(std::string titulo, std::string director, bool estreno, float precioBase, std::string tipoPelicula):
    codigo(autonumerico++),
    titulo(titulo),
    director(director), 
    estreno(estreno), precioBase(precioBase), 
