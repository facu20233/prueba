...
public:
	...
	void escribir();
	void escribirPelicula(); //*
	T Primer();
	int capacidad();
	bool pertenece(int X);
	int ContarX(const T &X); //*
	~Arreglo();
};
...
template <typename T>
void Arreglo<T>::escribirPelicula()
{
	cout << " CONTENEDOR Pelicula:" << endl;
	for (int i = 1; i < this->MAX; i++)
	{
		this->arreglo[i];
	}
...
template <typename T>
bool Arreglo<T>::pertenece(int X)
{
	int k = 1;
...
template <typename T>
int Arreglo<T>::ContarX(const T &X)
{
    int contador = 0;
    for (int i = 1; i < this->MAX; ++i)
    {
