...
productosSinBebidas(ProductoX, PrecioMin, PrecioMax):-
...
productosCanasta(ProductoX, PrecioDescuento, PrecioMax):-
...
total([], 0).
total([ProductoX|Resto], TotalProductos):-
    % producto(Categoria, ProductoX), %de mas
    precio(ProductoX, PrecioX),
    total(Resto, TotalResto),
    TotalProductos is TotalResto + PrecioX.
